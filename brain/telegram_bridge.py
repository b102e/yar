    ...
    async def _cmd_die(self, client: httpx.AsyncClient, chat_id: int, args: str) -> None:
        ...
        if not args_lower.startswith("confirm"):
            await self._send_message(client, chat_id,
                ...
                "После смерти остаётся только:\n"
                "• цепочка метаданных (когда агент действовал)\n"
                "• публичный ключ (для верификации)\n\n"
                "Личные данные, факты, гипотезы, эпизодическая память — "
                "будут перезаписаны нулями и удалены.\n\n"
                "Это необратимо.\n\n"
                "Для подтверждения:\n"
                "/die confirm [причина]"
            )
